...
module Cst = struct
  ...
  let struct_type ~name fields =
    node "type" [ wasm_var name; struct_descr fields ]

  let struct_subtype ~name ~sub fields =
    node "type"
      [ wasm_var name; node "sub" [ wasm_var sub; struct_descr fields ] ]

  let func_type ~name ~params ~result =
    let params = List.map (fun typ -> node "param" [ typ ]) params in
    node "type"
      [ wasm_var name; node "func" (params @ [ node "result" [ result ] ]) ]
...
and function_declarations env dacc (declarations : Flambda.function_declarations)
    : Cst.t list * DA.t =
  Variable.Map.fold
...
and function_declaration (env : Env.t) (dacc : DA.t) name
  ...
  let _closure_type_name, dacc =
    let param_set =
      List.fold_left
        (fun param_set param -> Variable.Set.add (Parameter.var param) param_set)
